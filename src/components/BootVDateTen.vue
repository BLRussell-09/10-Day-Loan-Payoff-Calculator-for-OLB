<template>
  <v-dialog ref="dialog" v-model="modal" :return-value.sync="date" persistent laxy full-width width="290px">
    <template v-slot:activator="{ on }">
      <v-text-field v-on:click="dateClick" id="payoff-date" v-model="dateFormatted" label="Payoff Date" prepend-inner-icon="event" box background-color="white" readonly v-on="on"/>
    </template>
